#           13.2. H2O DL Edmonson Grade prediction model - LICA
...
#pesudo and non-coding gene set of LICA, will pretend to predict the corresponding
#Edmonson Grade to that patient.
...
edmonson <- read_excel("edmonson.xlsx", na = "NA")
edmonson <- edmonson[,2]
edmonson <- as.tibble(edmonson)


ncRNA[,(dim(ncRNA)[2]+1)] <- edmonson
colnames(ncRNA) <- cnames
colnames(ncRNA)[dim(ncRNA)[2]] <- "edmonson"
Data <- ncRNA
...
datos_train$edmonson <-  as.factor(datos_train$edmonson)
datos_test  <- testing(split_inicial)
datos_test$edmonson <-  as.factor(datos_test$edmonson)

summary(datos_train$edmonson)
summary(datos_test$edmonson)
...
mojo_destination <- h2o.save_mojo(modelo_final, path = getwd(),filename = "modelo_pseudo+nc_nfold20_edmonson")
