#           13.4. H2O DL Vascular Invasion prediction model - LICA
...
#pesudo and non-coding gene set of LICA, will pretend to predict if that tumor has
#vascular invasion or not.
...
vascular_invasion <- read_excel("vascular_invasion.xlsx", na = "NA")
vascular_invasion <- vascular_invasion[,2]
vascular_invasion <- as.tibble(vascular_invasion)


ncRNA[,(dim(ncRNA)[2]+1)] <- vascular_invasion
colnames(ncRNA) <- cnames
colnames(ncRNA)[dim(ncRNA)[2]] <- "Vascular_invasion"
Data <- ncRNA
...
split_inicial <- initial_split(
  data   = Data,
  prop   = 0.8,
  strata = Vascular_invasion
)

datos_train <- training(split_inicial)
datos_train$Vascular_invasion <-  as.factor(datos_train$Vascular_invasion)
datos_test  <- testing(split_inicial)
datos_test$Vascular_invasion <-  as.factor(datos_test$Vascular_invasion)

summary(datos_train$Vascular_invasion)
summary(datos_test$Vascular_invasion)


#Raw data must be processed to remove NA, dummy variables, etc. A recipe has been
#developed.
transformer <- recipe(
  formula = Vascular_invasion ~ .,
  data =  datos_train
) %>%
...
# ==============================================================================
variable_respuesta <- 'Vascular_invasion'
...
mojo_destination <- h2o.save_mojo(modelo_final, path = getwd(),filename = "modelo_pseudo+nc_nfold20_vascular_invasion")
