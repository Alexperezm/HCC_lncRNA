#           13.3. H2O DL Fibrosis Grade prediction model - LICA
...
#Fibrosis Grade to that patient.
...
fibrosis <- read_excel("fibrosis.xlsx", na = "NA")
fibrosis <- fibrosis[,2]
fibrosis <- as.tibble(fibrosis)


ncRNA[,(dim(ncRNA)[2]+1)] <- fibrosis
colnames(ncRNA) <- cnames
colnames(ncRNA)[dim(ncRNA)[2]] <- "fibrosis"
Data <- ncRNA
...
datos_train$fibrosis <-  as.factor(datos_train$fibrosis)
datos_test  <- testing(split_inicial)
datos_test$fibrosis <-  as.factor(datos_test$fibrosis)

summary(datos_train$fibrosis)
summary(datos_test$fibrosis)
...
mojo_destination <- h2o.save_mojo(modelo_final, path = getwd(),filename = "modelo_pseudo+nc_nfold20_fibrosis")
