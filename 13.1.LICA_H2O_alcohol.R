#           13.1. H2O DL Alcohol consumption prediction model - LICA
...
#pesudo and non-coding gene set of LICA, will pretend to predict if that patient
#is alcohol consumer or not.
...
alcohol <- read_excel("alcohol.xlsx", na = "NA")
alcohol <- alcohol[,2]
alcohol <- as.tibble(alcohol)


ncRNA[,(dim(ncRNA)[2]+1)] <- alcohol
colnames(ncRNA) <- cnames
colnames(ncRNA)[dim(ncRNA)[2]] <- "alcohol"
Data <- ncRNA
...
datos_train$alcohol <-  as.factor(datos_train$alcohol)
datos_test  <- testing(split_inicial)
datos_test$alcohol <-  as.factor(datos_test$alcohol)

summary(datos_train$alcohol)
summary(datos_test$alcohol)
...
mojo_destination <- h2o.save_mojo(modelo_final, path = getwd(),filename = "modelo_pseudo+nc_nfold20_alcohol")
